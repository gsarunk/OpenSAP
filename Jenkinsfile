...
pipeline{
    ...
    stages{
        stage('CI-Unit Test/Coverage'){
            steps {
                bat "node newman run CodeCoverage.json --environment SAPEC8EnvCodeCoverage.json"
		   
            }
        }
	    stage('CI-Code Review'){
	    steps{
		bat "node newman run ATCCheckLegacy.json --environment SAPEC8EnvCodeCoverage.json"
	    }
	}
	
	stage('CD-Deploy To QA'){
	    steps{
		bat "node newman run DeployToQuality.json --environment SAPEC8EnvCodeCoverage.json"
	    }
	}
  
  stage('CI-End To End Test'){
	    steps{
		bat "node newman run E2ETest.json --environment SAPEC8EnvCodeCoverage.json"
	    }
	}
	}
}	
